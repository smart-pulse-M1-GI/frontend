  ...
  useEffect(() => {
    const fetchPatientInfo = async () => {
      try {
        ...
        // Récupérer les infos du patient depuis la liste des patients du médecin
        const patientsRes = await fetch(`${apiUrl}/api/user/my-patients`, {
          headers: getHeaders(),
        });
  ...
  useEffect(() => {
    const fetchSessions = async () => {
      ...
      try {
        ...
        console.log('📊 Chargement des sessions pour patient:', id);

        // Récupérer toutes les sessions
        const sessionsRes = await fetch(
          `${apiUrl}/api/v1/cardiac/patient/${id}/sessions`,
          {
            headers: getHeaders(),
          }
        );
        ...
        {filteredSessions.length === 0 ? (
          <Card>
            <CardContent className="p-12 text-center">
              ...
              <p className="text-sm text-muted-foreground">
                {selectedPeriod === 'all' 
                  ? 'Ce patient n\'a pas encore effectué d\'activité'
